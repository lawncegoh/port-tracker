...
export class PrismaRepo implements Repo {
  ...
  async saveProperty(property: RealEstateProperty): Promise<void> {
    ...
    const data = {
      ...
      rateSchedule: serializeRateSchedule(property.rateSchedule) ?? null,
      disbursementSchedule: serializeDisbursementSchedule(property.disbursementSchedule) ?? null,
      userId: this.userId,
    };

    if (existing) {
      await prisma.realEstateProperty.update({ where: { id: property.id }, data });
    } else {
      await prisma.realEstateProperty.create({ data: { ...data, id: property.id } });
    }
  }
